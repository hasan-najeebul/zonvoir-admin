<div class="card pt-4 mb-6 mb-xl-9">
    ...
    <div class="card-body pt-0 pb-5">
        <!--begin::Table-->
        <table class="table align-middle table-row-dashed gy-5" id="kt_table_partner_invoices" data-table-url="{{ route('user-management.invoice-list',$user->id)}}">
            <!--begin::Table head-->
            <thead class="border-bottom border-gray-200 fs-7 fw-bolder">
                <!--begin::Table row-->
                <tr class="text-start text-muted text-uppercase gs-0">
                    <th class="min-w-100px">{{__('messages.name')}}</th>
                    <th class="min-w-100px">{{__('messages.invoice_id')}}</th>
                    <th class="min-w-100px">{{__('messages.amount')}}</th>
                    <th class="min-w-80px">{{__('messages.status')}}</th>
                    <th class="min-w-100px">{{__('messages.duedate')}}</th>
